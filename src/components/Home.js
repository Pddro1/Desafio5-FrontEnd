...
const Home = () => {
  ...
  const saldoPeriodo = transferencias.reduce(
    (total, current) => total + current.valor,
    0
  );
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8080/api/transferencias?minDate=${dmin}&maxDate=${dmax}`
      )
      .then((response) => {
        ...
        <div className="mt-4">
          ...
          <Table striped bordered hover>
            ...
            <tbody>
              {transferencias
                .filter((transf) => {
                  return procurar.toLowerCase() === ""
                    ? transf
                    : transf &&
                        transf.nomeOperadorTransacao &&
                        transf.nomeOperadorTransacao
                          .toLowerCase()
